               ...
               std::vector<camera_params_t> &cameras)
{
    ...
    for (int i = 0; i < num_cameras; i++) {
        ...
        std::string basename_tmp = images[i].substr(0, last_dot);
        int last_slash = basename_tmp.rfind('/', basename_tmp.length()-1);
        std::string basename = basename_tmp.substr(last_slash+1);
